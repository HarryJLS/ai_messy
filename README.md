...
- **Dev Team**：多 Agent 团队编排，自动化串联预研→初始化→开发→优化全流程，支持跨项目代码参考
...
| | dev-team | `/dev-team` | 多 Agent 团队全流程编排（preview→init→next→simplifier→fixer） |
...
| 角色 | Agent | 职责 |
|------|-------|------|
| lead | 自身 | 编排调度、用户沟通、转达决策 |
| architect | general-purpose | 方案预研 + 项目初始化 |
...
```text
阶段 0: 团队初始化（识别需求、检测跳入点）
阶段 1: 方案预研（architect 执行 /plan-preview）
阶段 2: 项目初始化（architect 执行 /plan-init）
...
- 阶段 1：architect 探索参考项目代码流程，将关键文件路径写入 `task.md` 的 `references` 字段
