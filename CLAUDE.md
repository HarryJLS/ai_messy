...
- 版本号格式：`YYYYMMDD.number`（如 `20260321.1`、`20260321.2`）
- 每次 commit 时 number +1（如 `20260321.1` → `20260321.2`）
- 新的一天首次 commit 时，日期更新为当天，number 从 1 重新计数（如 `20260321.3` → `20260322.1`）
...
**1. 先思考再编码**
明确表达假设，不要默默推进。不确定时先提问，在实现前说明权衡和可选方案。

**2. 简单优先**
编写满足需求的最小代码。避免推测性功能、不必要的抽象、或针对低概率场景的错误处理。标准：资深工程师不会说"这过于复杂"。

**3. 外科手术式修改**
编辑代码时只改完成请求所必需的部分。匹配现有代码风格，不重构无关代码。检验标准：每一行改动都能直接追溯到用户的需求。

**4. 目标驱动执行**
在开始前将模糊需求转化为可验证的成功标准。例如："修复 bug" → "先写一个能复现 bug 的测试，再让测试通过"。这样可独立迭代并有清晰的检查点。
