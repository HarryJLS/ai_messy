...
**目标范围**：
- 如果任务含 `domain` 字段，只看 `domain=backend` 的任务；否则看所有任务
- 如果用户指定了 app 名（如 `/backend-single order-service`），进一步只看 `app=order-service` 的任务
...
- 传入 `domain=backend`
- 如果用户指定了 app（如 `/backend-single order-service`），同时传入 `app=order-service`

plan-next 会自动按过滤条件循环执行所有匹配的任务，包括 appPath 路由。

**执行步骤：**
1. 调用 `Skill("plan-next")`，传入 domain=backend（+ 可选 app 参数）
2. plan-next 内部按 TDD 流程循环完成所有匹配任务（READ → EXPLORE → PLAN → RED → IMPLEMENT → GREEN → COMMIT）
