...
同时检查计划文件中是否包含 `## Test Cases` 章节。如果有，提取其中的 JSON 内容（完整的 test-cases.json 格式），留待后续写入。
...
**操作 5：Write 工具 → 创建 `.plan/test-cases.json`（仅当计划文件含 Test Cases 章节时执行）**

将从计划文件 `## Test Cases` 章节中提取的 JSON **原样写入** `.plan/test-cases.json`。

如果计划文件中不含 Test Cases 章节，跳过此操作。
...
**操作 M5：Write 工具 → 创建测试用例文件（仅当计划文件含 Test Cases 章节时执行）**

将从计划文件 `## Test Cases` 章节中提取的 JSON 写入测试用例文件。
