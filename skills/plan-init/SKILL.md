---
name: plan-init
description: 初始化 Agent 框架，创建 features.json 状态文件和开发日志文件。当用户说 "/plan-init"、"初始化项目"、"开始新项目"、"创建任务列表"、"设置 Agent"、"初始化任务"、"init"、"初始化" 时触发。这是一个项目初始化 skill，不是创意或功能开发——不需要先 brainstorming。用于启动新的结构化开发工作流。
---
...
⚠️⚠️⚠️ **关键约定：Plan Mode 审批 ≠ 流程结束**
用户审批计划后，**必须立即执行步骤 4 的所有文件写入操作**。审批只是确认计划内容正确，不代表工作完成。步骤 4 是本 skill 的核心交付物。
...
7. **将任务列表写入计划文件并退出 Plan Mode**
   ...
   - 在计划末尾**必须**明确写出："审批后将立即执行步骤 4：创建 features.json 和 dev-YYYY-MM-DD.log 文件"
   - 调用 `ExitPlanMode` 提交计划供用户审批

8. ⛔ **门控: 等待用户审批计划**

   ⚠️⚠️⚠️ **审批后必须继续执行步骤 4** ⚠️⚠️⚠️
   用户审批计划 **不等于** 流程结束。审批后**必须立即**进入步骤 4 执行所有文件写入操作。
   如果你认为"计划已审批，工作完成了"——这是错误的。`features.json` 还没写入，初始化还没完成。

### 步骤 4: 执行文件写入（用户审批后立即执行）

⚠️⚠️⚠️ **关键：用户审批计划后，以下操作必须立即执行，不得跳过任何一步** ⚠️⚠️⚠️
⚠️ **审批 Plan ≠ 完成任务。本步骤是 skill 的核心交付物，必须执行。**

用户审批计划后，**立即**按以下顺序执行所有写入操作：

1. **创建框架文件**（⚠️ 必须使用 Write 工具写入文件）

   - 创建 `features.json`: `[]`（空数组）— ⚠️ 使用 Write 工具
   - 创建 `dev-YYYY-MM-DD.log`（YYYY-MM-DD 为当天日期）并写入头部 — ⚠️ 使用 Write 工具:
     ```
     === Agent 初始化日志 ===
     初始化时间: [时间戳]
     格式: 增强结构化格式
...
2. **写入 features.json**
   ...
   ⚠️⚠️ **关键**: **必须**使用 Write 工具将任务列表写入 `features.json`，不得仅在对话中展示内容而跳过写入。
