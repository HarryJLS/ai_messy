---
name: backend-test
description: 后端测试验证，基于 .plan/features.json 已完成任务驱动：运行单元测试 + 启动服务验证 API 契约 + 逐任务检查验收标准。当用户说 "/backend-test"、"后端测试"、"跑后端测试"、"验证后端"、"API 测试" 时触发。
---
...
```
读取 features.json → 框架检测 → 运行单元测试 → 启动服务 → API 契约验证 → 验收标准检查 → 生成报告 → 清理
```
