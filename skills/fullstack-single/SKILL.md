...
| 有 `.plan/features.json`、有 backend 未完成任务、`.plan/dev-*.log` 中有开发日志 | 阶段 2（中断恢复） |
| 有 `.plan/features.json`、有 backend 未完成任务、无开发日志 | 阶段 1（需先 plan-write） |
...
1. 读取 features.json，统计全部任务状态：
   - 如有跳过的任务，输出警告："⚠️ X 个任务被跳过，建议检查后再继续"
2. **后端 Build 验证**：检测后端构建工具并运行
   | 检测条件 | 构建命令 |
   |----------|---------|
...
3. **前端 Build 验证**：检测前端构建工具并运行
   | 检测条件 | 构建命令 |
   |----------|---------|
   | `package.json` 有 build script | `npm run build` |
   | `vite.config.*` 存在 | `npx vite build` |
   | `next.config.*` 存在 | `npx next build` |
4. **Test 验证**：运行后端 + 前端测试（如有测试脚本）
5. 输出结果："快速验证: 后端 Build [PASS/FAIL] | 前端 Build [PASS/FAIL] | Test [PASS/FAIL]"
6. 任何 Build 失败 → AskUserQuestion（展示错误日志，询问是否修复后继续或跳过验证）
7. Test 失败（仅新失败的用例） → AskUserQuestion
8. 全部通过 → 进入阶段 4
...
3. 先用 `git diff` 确定本次开发修改的文件范围（后端 + 前端），将文件列表作为优化目标
