...
**前置条件**：需先运行 `/plan-init` 完成任务分解并审批。task.md 中需包含 `domain` 列区分 backend/frontend 任务。

**与 fullstack-team 的区别**：去掉方案预研（plan-preview + 设计系统）、方案审查（plan-reviewer）、全量验证（build-fixer）、多维 CR（reviewer/blind-reviewer/security-reviewer）、pr-description.md 生成、De-Sloppify 检查。保留核心开发流水线。
...
循环执行 `Skill("plan-next")`，直到所有 `domain=backend` 任务的 `passes` 都为 `true`。

**执行步骤：**
1. 读取 `.plan/features.json`，确认还有 `passes: false` 且 `domain=backend` 的任务
2. 调用 `Skill("plan-next")` 执行下一个后端任务
3. plan-next 内部按 TDD 流程完成（RED → GREEN → COMMIT）
4. 检查 `.plan/features.json`，如仍有 backend 未完成任务则继续循环
5. 后端任务全部 `passes: true` → 进入阶段 3

**domain 判断**：读取 .plan/features.json 中每个任务的描述，根据 task.md 中的 domain 列判断属于 backend 还是 frontend。按 .plan/features.json 中的顺序执行，跳过 domain=frontend 的任务。
...
循环执行 `Skill("plan-next")`，直到所有 `domain=frontend` 任务的 `passes` 都为 `true`。

**执行步骤：**
1. 读取 `.plan/features.json`，确认还有 `passes: false` 且 `domain=frontend` 的任务
2. 调用 `Skill("plan-next")` 执行下一个前端任务
3. 前端调用后端 API 时使用已实现的真实接口，不用 mock
4. plan-next 内部按 TDD 流程完成（RED → GREEN → COMMIT）
5. 检查 `.plan/features.json`，如仍有 frontend 未完成任务则继续循环
6. 前端任务全部 `passes: true` → 进入阶段 4
