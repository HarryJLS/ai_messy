...
**与 dev-team 的核心区别**：dev-team 围绕"通用代码"设计（plan-preview 探索代码库），本 skill 面向"前端开发"场景——阶段 1 用设计系统生成 + UI 方案替代代码探索，polisher 增加 UI/UX 规范检查，全程按检测到的前端框架（React/Vue3/Vue2）分支处理。
...
### 阶段 1: 设计系统生成 + UI 方案预研（替代 dev-team 的 plan-preview）

**这是与 dev-team 的核心区别。** 不走通用的 plan-preview，而是先生成设计系统、构建 UI 方案，再输出 task.md。
...
**1c. 生成 task.md + pr-description.md**

1. 综合 1a 和 1b 的结果，生成 `task.md`：
...
来源：从 task.md 的设计系统和技术方案提取，**不含具体文件路径和代码细节**。

3. 确认 `task.md` 和 `pr-description.md` 已生成 → 进入阶段 1.5
...
**优势**：lead 在阶段 1 亲历了设计系统生成和 UI 方案全过程，此阶段大部分门控可基于已有上下文直接通过，无需重复询问用户。plan-init 和 plan-write 分离后，中断恢复更精确：计划已审批但未写入时可直接从 2b 恢复。
...
| E2E | 调用 `Skill("e2e-test")` | 页面可访问、无控制台错误、响应式正常 |
...
**这是与 dev-team 的另一个核心区别。** polisher 在标准代码打磨基础上，增加 UI/UX 规范检查。
