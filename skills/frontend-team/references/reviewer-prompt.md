...
- 只报告置信度 >80% 的问题
- 跳过代码风格偏好（除非违反项目规范）
- 相似问题合并（如"5 个组件缺少 key 属性"而非 5 条单独发现）

## 严重等级

| 等级 | 范围 |
|------|------|
| CRITICAL | 安全漏洞（XSS、未校验输入、敏感数据暴露、dangerouslySetInnerHTML） |
| HIGH | 逻辑错误（竞态条件、陈旧闭包、缺少测试覆盖、useEffect 依赖缺失） |
| MEDIUM | 性能问题（不必要的重渲染、大列表未虚拟化）、代码质量 |
| LOW | 命名、注释、格式 |
...
3. 按以下维度审查：
   ...
   **前端专项维度**（详见 `code-review/frontend.md`）：
