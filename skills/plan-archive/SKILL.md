---
name: plan-archive
description: 归档已完成的功能、日志和相关文档。当用户说 "/plan-archive"、"归档项目"、"清理工作区"、"备份任务" 时触发。用于里程碑完成后清理工作区，开始新迭代。自动归档并删除原文件。
---
...
选项：
  - "是，移动到归档" → 移动到 archive/docs/agents/，删除原文件
  - "是，复制到归档" → 复制到 archive/docs/agents/，保留原文件
  - "否，跳过" → 不归档文档
```

⛔ **等待用户回答**，然后执行选定操作

**如果 `agents/` 目录不存在或为空**：显示消息后继续阶段 4
