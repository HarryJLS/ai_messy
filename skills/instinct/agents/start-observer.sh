#!/bin/bash
# Instinct - Observer Agent 启动器
#
# 启动后台 observer agent 分析观察数据并创建 instincts。
# 使用 Haiku 模型保持成本效率。
#
# 用法：
#   start-observer.sh          # 启动当前项目的 observer
#   start-observer.sh stop     # 停止 observer
#   start-observer.sh status   # 检查 observer 状态
  ...
  status)
    if [ -f "$PID_FILE" ]; then
      ...
      if kill -0 "$pid" 2>/dev/null; then
        ...
        instinct_count=$(find "$INSTINCTS_DIR" -name "*.yaml" -o -name "*.md" 2>/dev/null | wc -l)
