#!/usr/bin/env python3
"""
Instinct CLI - 管理 instincts 的命令行工具

支持项目隔离：不同项目获得不同的 instincts，全局 instincts 通用应用。

Commands:
  status   - 显示所有 instincts（项目级 + 全局）
  import   - 从文件/URL 导入 instincts
  export   - 导出 instincts 到文件
  evolve   - 聚类 instincts → 生成 skill/command/agent
  promote  - 提升项目级 instinct 到全局
  projects - 列出已知项目及统计
"""
...
def main() -> int:
    _ensure_global_dirs()
    parser = argparse.ArgumentParser(description='Instinct CLI - 管理 instincts（项目隔离 + 全局）')
